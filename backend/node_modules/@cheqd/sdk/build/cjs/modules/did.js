...
class DIDModule extends _1.AbstractCheqdSDKModule {
    ...
    static async validateSpecCompliantPayload(didDocument) {
        ...
        const protoService = didDocument?.service?.map((s) => {
            return index_1.Service.fromPartial({
                id: s?.id,
                serviceType: s?.type,
    ...
    static async toSpecCompliantPayload(protobufDidDocument) {
        ...
        const service = protobufDidDocument.service.map((s) => {
            if (s.serviceType === types_1.ServiceType.LinkedDomains)
                protobufDidDocument.context = [...protobufDidDocument.context, exports.contexts.LinkedDomainsContext];
            return {
                id: s.id,
                type: s.serviceType,
                serviceEndpoint: s.serviceEndpoint,
    ...
    static async validateAuthenticationAgainstSignaturesKeyRotation(didDocument, signatures, querier, previousDidDocument, externalControllersDidDocuments) {
        ...
        const controllerRotation = !didDocument.controller.every((c) => previousDidDocument.controller.includes(c)) ||
            !previousDidDocument.controller.every((c) => didDocument.controller.includes(c));
        // define rotated controllers
        const rotatedControllers = controllerRotation
            ? previousDidDocument.controller.filter((c) => !didDocument.controller.includes(c))
            : [];
        ...
        if (!querier)
            throw new Error('querier is required for external controller validation');
        // get external controllers
        const externalControllers = didDocument.controller
